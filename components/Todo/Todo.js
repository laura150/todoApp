  ...
  return (
    <div className={styles.App}>
     ...
     <div className={styles.showtasksection}>
      <div className={styles.task}>{purpleset||greenset? null : `showing ${list.length} task${list.length > 1? 's': ''}`}
                            {purpleset? `filtering and showing ${list.length} task${list.length > 1? 's': ''}`: null} 
                            {greenset? `filtering and showing ${list.length} task${list.length > 1? 's': ''}` : null}
                           </div>
        <div>
